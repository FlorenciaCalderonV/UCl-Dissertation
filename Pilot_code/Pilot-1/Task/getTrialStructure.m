...
%Order of tasks alternates after first task as blocks progress.
    ...
    for  r = 1:Nr
        ...
        if order == 1 %counterbalancing makes L/R first
            ...
            trialMatrixTemp(idxR,3) = repmat([1;2],[Nt/2,1]); %Angle: 1 = 60 deg, 2 = 160 deg
            trialMatrixTemp(idxR,4) = repmat([1;1;2;2],[Nt/4,1]); %V/O: 1 = V; 2 = O
            trialMatrixTemp(idxR,5) = repmat([1;1;1;1;2;2;2;2],[Nt/8,1]); %Avatar L/R: 1 = Left, 2 = Right
            %shuffle trials
            ...
            trialMatrixTemp(idxR,3) = repmat([1;2],[Nt/2,1]); %Angle: 1 = 60 deg, 2 = 160 deg
            trialMatrixTemp(idxR,4) = repmat([1;1;2;2],[Nt/4,1]); %L/R: 1 = L; 2 = R
            trialMatrixTemp(idxR,5) = repmat([1;1;1;1;2;2;2;2],[Nt/8,1]); %Avatar L/R: 1 = Left, 2 = Right
            %shuffle trials
            shufIdx = idxR(randperm(length(idxR)));
            trialMatrix(shufIdx,:) = trialMatrixTemp(idxR,:);
